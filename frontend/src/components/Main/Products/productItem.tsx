import { ShoppingCartOutlined, StarFilled } from '@ant-design/icons';
import { Button, Card, Rate, Typography } from 'antd';
import { FC } from 'react';
import { Link } from 'react-router-dom';
import { Product } from 'types/product';

type Props = {
  item: Product;
  onAdd: ( item: Product ) => void;
};

const ProductItem: FC<Props> = ({ item, onAdd }) => {
  return (
    <div>
      <Card
        bordered
        cover={
          <div style={{ height: '250px', width: '250px', display: 'flex',margin: 'auto', padding: '5px'}}>
            <img
              style={{ height: '100%', maxWidth: '100%', margin: 'auto', display: 'flex' }}
              alt='example'
              src={
                item.photoURL ||
                'data:image/png;base64,'
              }
            />
          </div>
        }
      >
        <Typography.Title level={4}>
          <Link style={{ color: 'black' }} to={`/product/${item.id}`}>
            {item.name}
          </Link>
        </Typography.Title>
        <Typography.Paragraph style={{ marginBottom: 0 }}>
          <Rate
            disabled
            value={item.rating}
            style={{ fontSize: 15, marginBottom: 0 }}
            character={<StarFilled style={{ width: '0' }} />}
          />
        </Typography.Paragraph>
        <Typography.Title
          level={3}
          style={{ marginTop: '10px', marginBottom: '-30px' }}
        >
          <Typography.Paragraph italic>
            {item.price}₽
            <Button onClick={() => onAdd(item)} type="primary" icon={<ShoppingCartOutlined />} size='middle' style={{marginLeft: '100px'}} />
          </Typography.Paragraph>
        </Typography.Title>
      </Card>
    </div>
  );
};

export default ProductItem;
